...
def New_Collective_Influence(G):
    ...
    for nid in G_CI_value.keys():
        ...
        #newCI[nid]=round(float(0.1*G_Balance[nid]/float(1+G_c[nid])*G_CI_value[nid]+0.9*G_Strength[nid]*G_CI_value[nid]),3)
        #print nid, newCI
        #newCI[nid]=round(float((1.0*G_Balance[nid]+0*G_Strength[nid])*G_CI_value[nid])/float(1+G_c[nid]),3)
        #newCI[nid]=round(float((0*G_Balance[nid]+1.0*G_Strength[nid])*G_CI_value[nid])/float(1+G_c[nid]),3)
...
G_init=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\Dataset_csv\SawmillTTTTT.csv")
...
csv_data = pd.read_csv(r'C:\Users\Administrator\Desktop\3th_result\SIR_matlab\SawmillTTTT\SawmillTTTT (2.1-3).csv')
#print csv_data
sir_node=[]
...
labels = ['0.21','0.22','0.23','0.24','0.25','0.26','0.27','0.28','0.29','0.30']
fig=plt.figure()
