...
def New_Collective_Influence(G):
    ...
    for nid in G_Balance.keys():
        ...
        #newCI[nid]=round(float(0.5*G_Strength[nid]/float(1+G_c[nid])*G_CI_value[nid]+0.5*G_Balance[nid]*G_CI_value[nid]),3)
        newCI[nid]=round(float(0.99*G_Balance[nid]/float(1+G_c[nid])*G_CI_value[nid]+0.01*G_Strength[nid]*G_CI_value[nid]),3)
...
#G_init=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\karate1.csv")
#G_init=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\Dataset_csv\Email.csv")
G_init=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\Dataset_csv\metabolic.csv")
#print G_init.edges()
...
for results_list in Methods_results_List:
    ...
    G=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\Dataset_csv\metabolic.csv")
    largest_cc_list=[]
    H_list=[]
    remove_rate=[]
    #print results_list
...
a=np.array(remove_rate,dtype=np.float64)
b=np.array(max_connected_component['newci91'],dtype=np.float64)/np.array(G_init.number_of_nodes())
print len(a),len(b)
print max_connected_component['newci91']
...
ax1.plot(np.array(remove_rate,dtype=np.float64) ,np.array(max_connected_component['newci91'],dtype=np.float64)/np.float64(G_init.number_of_nodes()),'y-',label='NewCI_B/R')
ax1.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['newci19'],dtype=float)/float(G_init.number_of_nodes()),'k-',label='NewCI_S/R')
...
#ax1.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['degree'],dtype=float)/float(G_init.number_of_nodes()),'k--',label='DC')
ax2.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['between'],dtype=float)/float(G_init.number_of_nodes()),'c--',label='BC')
ax2.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['closeness'],dtype=float)/float(G_init.number_of_nodes()),'y--',label='CC')
ax2.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['pagerank'],dtype=float)/float(G_init.number_of_nodes()),'m--',label='EC')
ax2.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['kcore'],dtype=float)/float(G_init.number_of_nodes()),'b--',label='k-core')
ax2.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['ci'],dtype=float)/float(G_init.number_of_nodes()),'g-',label='CI')
ax2.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['newci'],dtype=float)/float(G_init.number_of_nodes()),'r-',label='NewCI')
