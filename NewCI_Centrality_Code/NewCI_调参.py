...
def New_Collective_Influence(G):
    ...
    for nid in G_Balance.keys():
        ...
        newCI19[nid]=round(float((0.1*G_Balance[nid]+0.9*G_Strength[nid])*G_CI_value[nid])/float(1+G_c[nid]),3)
        #print nid, newCI

    return newCI,newCI91,newCI19
...
newci,newci91,newci19=New_Collective_Influence(G_init)
...
methods_list=['degree','between','closeness','pagerank','kcore','ci','newci','newci91','newci19']
...
Methods_results_List=[res_list_d,res_list_b,res_list_c,res_list_r,res_list_k,res_list_CI,res_list_NewCI,res_list_NewCI91,res_list_NewCI19]
#methods_id=0
for results_list in Methods_results_List:
    methods_id=Methods_results_List.index(results_list)
    #G=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\karate.txt")
    G=createGraph(r"C:\Python27\sjxwork\NewCI_Centrality_Code\Dataset_csv\Email.csv")
    largest_cc_list=[]
    H_list=[]
    remove_rate=[]
    #print results_list
...
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['degree'],dtype=float)/float(G_init.number_of_nodes()),'k--',label='DC')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['between'],dtype=float)/float(G_init.number_of_nodes()),'c--',label='BC')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['closeness'],dtype=float)/float(G_init.number_of_nodes()),'y--',label='CC')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['pagerank'],dtype=float)/float(G_init.number_of_nodes()),'m--',label='PageRank')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['kcore'],dtype=float)/float(G_init.number_of_nodes()),'b--',label='k-core')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['ci'],dtype=float)/float(G_init.number_of_nodes()),'g--',label='CI')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['newci'],dtype=float)/float(G_init.number_of_nodes()),'r--',label='NewCI')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['newci91'],dtype=float)/float(G_init.number_of_nodes()),'k-',label='NewCI10')
ax.plot(np.array(remove_rate,dtype=float) ,np.array(connected_components_nums['newci19'],dtype=float)/float(G_init.number_of_nodes()),'b-',label='NewCI19')


ax.legend(loc='best')
ax.set_xlabel("q",fontstyle='italic')
ax.set_ylabel("G(q)",fontstyle='italic')
fig.show()
fig1=plt.figure()
...
ax1.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['pagerank'],dtype=float)/float(G_init.number_of_nodes()),'m--',label='PageRank')
...
ax1.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['ci'],dtype=float)/float(G_init.number_of_nodes()),'g--',label='CI')
...
ax1.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['newci91'],dtype=float)/float(G_init.number_of_nodes()),'k-',label='NewCI10')
ax1.plot(np.array(remove_rate,dtype=float) ,np.array(max_connected_component['newci19'],dtype=float)/float(G_init.number_of_nodes()),'b-',label='NewCI19')

ax1.legend(loc='best')
ax1.set_xlabel("q",fontstyle='italic')
ax1.set_ylabel("C(q)",fontstyle='italic')
fig1.show()
raw_input()